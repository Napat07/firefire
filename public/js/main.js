...
function contactRecordFromSnapshot(snapshot) {
  ...
  record += '<p class="card-title">Name: ' + contact.name + '</p>';
  record += '<p><i class="material-icons">Email</i> : ' + contact.detail.email + '</p>';
  record += '<p><i class="material-icons">Phone</i> : ' + contact.detail.phone + '</p>';
  record += '<button class="button" href="#" onclick="removeContact(\'' + key + '\')">Remove</</button>';
  record += '</div></div>';
